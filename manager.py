...
class Manager:
    ...
    def initial_page(self, tl):
        print("Welcome to the Zendesk CLI ticket viewer")

        # setting up initial page
        ticket_count = self.query_api(f"{self.web_address}/api/v2/tickets/count", self.user_in, self.api_token)
        ticket_count = int(ticket_count["count"]["value"])
    ...
    # provides API connection functionality
    ...
    # updates the pagination cursors
